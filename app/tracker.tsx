...
export function BusTracker() {
  ...
  useEffect(() => {
    ...
    const onBeforeInstall = (event: Event) => { event.preventDefault(); setInstallPrompt(event as unknown as { prompt: () => Promise<void>; userChoice: Promise<{ outcome: string }> }); };
  ...
  return <main className="app-shell" dir={language === "ur" ? "rtl" : "ltr"}>
    <header className="topbar"><a className="brand" href="#top" aria-label={language === "ur" ? "بس کہاں ہے؟" : "Bus Kahan Hai?"}><img className={`brand-logo ${language === "ur" ? "brand-logo-ur" : ""}`} src={language === "ur" ? logoUr.src : logoEn.src} alt={language === "ur" ? "بس کہاں ہے؟" : "Bus Kahan Hai?"} /></a><div className="topbar-actions">{!appInstalled && (installPrompt || isIOS) && <button className="install-app" onClick={() => { if (installPrompt) void installApp(); else setShowIOSHelp(true); }} aria-label={language === "ur" ? "ایپ ڈاؤن لوڈ کریں" : "Download App"}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v12m0 0l-4-4m4 4l4-4M5 21h14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg><span>{language === "ur" ? "ایپ ڈاؤن لوڈ کریں" : "Download App"}</span></button>}<div className="language-toggle" aria-label="Choose language"><button className={language === "en" ? "active" : ""} onClick={() => setLanguage("en")}>English</button><button className={language === "ur" ? "active" : ""} onClick={() => setLanguage("ur")}>اردو</button></div><span className={`feed-pill ${visibleBusCount ? "is-live" : ""}`}><i />{visibleBusCount ? t.live : t.online}</span></div></header>
    {showIOSHelp && <div className="ios-help-backdrop" role="dialog" aria-modal="true" onClick={() => setShowIOSHelp(false)}><div className="ios-help" onClick={(event) => event.stopPropagation()}><button className="ios-help-close" aria-label="Close" onClick={() => setShowIOSHelp(false)}>×</button><h3>{language === "ur" ? "ہوم اسکرین پر شامل کریں" : "Add to Home Screen"}</h3><p>{language === "ur" ? "iPhone پر ایپ انسٹال کرنے کے لیے:" : "To install the app on iPhone:"}</p><ol><li>{language === "ur" ? "Safari میں نیچے شیئر بٹن دبائیں" : "Tap the Share button in Safari"} <span className="ios-share" aria-hidden="true">⬆️</span></li><li>{language === "ur" ? "\"Add to Home Screen\" منتخب کریں" : "Choose “Add to Home Screen”"}</li><li>{language === "ur" ? "\"Add\" دبائیں — ایپ \"Bus Kahan Hai?\" کے نام سے شامل ہو جائے گی" : "Tap “Add” — it appears as “Bus Kahan Hai?”"}</li></ol></div></div>}
